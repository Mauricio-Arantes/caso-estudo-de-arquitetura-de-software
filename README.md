...
O estudo, se dedica a tratar sobre uma problemática envolvendo 3 bases de dados distintas, sendo cada qual atender
demandas distintas. 

O primeiro banco, armazena dados sensíveis, e precisa ser muito bem protegida e não precisa ser tão rápido de ser lida.

O segundo banco, também armazena dados sensíveis, mas não tão sensíveis quanto o anterior, porém, precisa de um acesso mais rápido.
Outro ponto característico desta base, é que a um serviço de inteligência artificial que irá consumir esses dados para fim de machine learning.
...
resolvi montar fluxograma para melhor representar como esses serviços podem se comunicar:
...
A importância do cacheamento, se faz necessário, a fim de poupar o banco de grandes processos de leitura, (o que comumente representa a maioria das operações exercida no banco de dados), novamente, solução necessária a ser aplicado quando se pensa em escalabilidade e velocidade.
...
Em suma, se não houver necessidade de queries muito pesadas, respostas repetitivas, ou queries (pesquisas) não muito pesadas, o Redis atende melhor e é mais performática que o ElasticSearch nesse sentido. Agora, se o cenário for o contrário, a adoção do ElasticSearch faz-se mais sentido.

## Escolha do banco de dados.
A escolha do banco, PostgreSQL é uma das melhores opções no momento, banco SQL consegue reunir alta escalabilidade, velocidade de resposta, confidenciabilidade dos dados e baixo custo de operação.
...
O lado negativo dessa infraestrutura segurança, inicialmente é o preço, que pode ficar bem alto, quando usado em massa (vez que a cobrança é realizada baseado no tempo do processamento e numero de chamadas), em segundo, performance, devido o tempo necessário para garantir que todas as chaves estão corretas para dar continuidade no pedido, e é agora que o “KMS” age, aprimorando a velocidade que este processo de segurança toma.

## Uso do conceito de CQRS para leitura e gravação.
Ter N serviços consumindo um mesmo banco de dados que serve para leitura e gravação, pode ocasionar muitos problemas de performance e até causar inconsitencia no banco de dados por um dump indesejado de dados, e ainda causar diversos problemas de performance. 

A aplicação deste conceito, visa a dividir a responsabilidade de gravação e leitura de forma conceitual e física, o que isto significa? Que os meios para gravar e obter dados diferem, que as consultas são feitas forma síncrona em uma base dessincronizada (Redis ou ElasticSearch neste caso) e as gravações de forma assíncrona em um banco normalizado (bancos que possuem referencias entre tabelas ou objetos, como PostgreSQL ou MongoDb)
